...
- **2026-07-16 — Phase 14.8**：EdgeOne Pages cloudFunctions 修复。**三阶根因**：14.7 修了 standalone，但 EdgeOne Pages 用自己的 bundler（不走 .next/standalone/，走 .edgeone/cloud-functions/ssr-node/），**不读 Next.js 的 standalone 产物**。默认 webpack 打包所有依赖，@prisma/client 含 native module（.node binary）被错误打包 → binary 丢失 → 500。**修复**：edgeone.json 加 `cloudFunctions.nodejs.externalNodeModules = ["@prisma/client", "prisma", "bcryptjs"]` + `includeFiles` 显式列出 8 个文件（3 binary × 2 路径 + schema.prisma + package.json）。**参考**：EdgeOne Pages 文档 https://pages.edgeone.ai/document/edgeone-json `cloudFunctions.nodejs.externalNodeModules` 字段。**验证**：0 type errors + 132/132 vitest。commit 7b7a9ed push。**固化**：bug-015-edgeone-cloudfunctions-external-modules.md。**教训铁三角**：(1) schema.prisma binaryTargets (2) next.config.js outputFileTracingIncludes (3) edgeone.json cloudFunctions.externalNodeModules + includeFiles — 任一缺失 → production 500。
