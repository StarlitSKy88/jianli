...
- **2026-07-16 — Phase 14.9**：EdgeOne Pages 无自动 rebuild 真相。**四阶根因（终极）**：13/14/15 三个修复 commit 都 push 到 master，但 EdgeOne Pages 没有 GitHub webhook 自动 rebuild 机制，prod 永远跑昨天的代码（sitemap lastmod 2026-07-15 vs git latest 2026-07-16，6+ 小时差距）。**修复**：用户手动去控制台 → 项目 → 部署 → 重新部署 → 等待 build。**新增 checklist**：部署后必跑 `curl sitemap.xml | grep lastmod` 健康检查。**教训**：dev pass + prod 永远停在过去 = 最阴险状态。文档 EDGEONE_DEPLOY.md § 6.2 更新。**固化**：bug-016-edgeone-no-auto-redeploy.md。**长效解**：Phase 15+ 配 edgeone CLI 或 GitHub Actions 自动 deploy。
