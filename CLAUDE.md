...
| Phase 14.8 Round 8A SSE abort 健壮性 + E17 识别 follow-up | ✅ 完成（2026-07-24） | **17/17 stress PASS, 0 FAIL, 1 SKIP**。**Round 8A E16 修✅**：client --max-time 0.05 早断后,`prisma.message.create(INTERVIEWER)` 在 `controller.enqueue` 之前写入,enqueue 单独 try/catch 吞 transport 异常 → 不再有 ghost assistant message。**Round 9 E17 修✅**：把 finish 评分路径从 `ReadableStream.start()` try 块**整体移出**为独立 `runFinishPipeline()` 函数,在 route handler 顶层 `void runFinishPipeline(...).catch(...)` 启动 fire-and-forget。**根因**：Next.js 14 SSE Response 在 stream 关闭时才返回 200,stream 内 await chain 在 client abort 时被 AbortSignal 串 cancel,`prisma.interview.update(COMPLETED)` 之后的 5 维度 scoreOne + saveReport + totalScore 整链路被跳过。**为什么 fire-and-forget 逃出 cascade**：独立 microtask 与 stream lifecycle 无引用关系,Next.js 不把它视作 stream 内 await。**比 cancel hook 更稳**：cancel 只在 client abort 时触发,正常完成时 finish 路径分裂两处难维护;fire-and-forget 单一路径覆盖所有 finish=true 场景。**测试同步**：E8/E10 sleep 3→8 (finish 评分 fire-and-forget 需 ~5s),E17 sleep 8→15 (POST handler 等 stream 关闭 8-13s),python JSON 解析 `strict=False` (response 含中文 prompt 嵌 \n 控制字符)。**Loop 纪律兑现**：上轮写 follow-up 卡 → 这轮在稳定 env 修完,一次 loop 收敛。|
| Phase 14.9 Phase B 10 真实用户全功能压测 | ✅ 完成（2026-07-24） | **`tests/stress/phase-b-10users.sh`** 新建 + **10 / 10 用户全过 0 阻塞** 🎉。10 画像覆盖 byte/ali/tencent/bili × P4-P8 × 后端/前端/算法/PM/QA/移动/35+。每用户跑完整 9 步: 注册→验证码→简历→面试创建→5 轮对话 (SSE)→finish 评分 (fire-and-forget)→评分报告 (5 维度 unique≥6)→付费 (mock 200 granted=1)→反馈 (200 accepted)→登出 (200 loggedOut)。**U3 P7 算法 / U7 P8 系统架构 / U10 P6 35+ 跨界运营** 三个**主目标画像**报告 score 77/82/79 ✅。**MVP 真实可用性里程碑达成** — 上一轮 Round 9 修了高危 SSE finish-score-lost bug,本轮用 **真实用户画像主动找 bug** 而非被动修,结果 0 个 user-blocking bug。**测试基础设施同步加固**：脚本里 pay-confirm 加 `-w '%{http_code} __TIME__:%{time_total}'` (Round 10 B-10-3 修: silent fail 时 stdout 空字符串被 grep 漏掉,现在 -o body file 分离 stdout/stderr/HTTP),get-verify-code 加 -m 10 + dump raw response (B-10-4 修: dev cold start 5002ms 偶发空响应诊断)。**bug 卡**: 30 → 31 (+1 phase-b-no-blocking-bugs 总结卡)。**SSE 17/17 不变** + **Phase B 10/10 新增** = 27 个端到端边界+真实用户用例都过。type-check 0 errors / vitest 232/232 passed 不变。
...
| `pnpm test` (vitest) | 232/232 passed（Round 5 +5：scoring-differentiation）|
...
| 知识卡 | patterns **12** + bugs **30** + recipes **2** = 44 张（Phase B +1：phase-b-10users-no-blocking-bugs 总结）|
...
| SSE 边界压测 | tests/stress/sse-boundary-tests.sh **17/17 passed, 0 FAIL, 1 SKIP**（Round 6 E1-E12 + Round 7 E13/14/15 + Round 8A E16 ghost-write + Round 9 E17 finish-score-lost）|
| Phase B 10 用户全功能 | tests/stress/phase-b-10users.sh **10 / 10 = 0 阻塞**（U1-U10 全覆盖,每用户 9 步全通）|
