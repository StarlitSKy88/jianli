...
- **2026-07-16 — Phase 14.7**：standalone outputFileTracingIncludes 修复。**二阶根因**：14.6 修了 schema binaryTargets，但 next.config.js 配 `output: 'standalone'`，standalone 只追踪 native binary，其他 3 个 linux engine **没进 .next/standalone build 产物**。pnpm 雪上加霜：真实文件在 `.pnpm/@prisma+client@*/...` 真实路径，`node_modules/.prisma/client/` 是 symlink，outputFileTracing 不解析 → 必须用 `.pnpm` 真实路径。**修复**：next.config.js 加 outputFileTracingIncludes 显式追踪 7 个文件（3 binary × 2 路径 + schema.prisma）。**验证**：`pnpm build` 后 `find .next/standalone -name "libquery_engine*"` 输出 3 个 binary（darwin-arm64 + linux-musl-openssl-3.0.x + rhel-openssl-3.0.x）+ 132/132 vitest + 0 type errors。commit b7905c5 push。**固化**：bug-014-prisma-standalone-multi-binary.md。**教训**：Prisma + Next.js standalone 部署 = schema.binaryTargets + next.outputFileTracingIncludes 必须同步维护；pnpm symlink 陷阱；dev pass + prod 500 = 最危险 false positive。
